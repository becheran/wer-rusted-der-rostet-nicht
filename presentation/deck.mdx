...
- Feature: Vieles zur Kopmpilierzeit geprüft
- Oft kompilieren!
...
Ein weiterer Vorteil der `match` Operation ist, dass **zur Compilezeit geprüft** wird, ob alle 
Optionen behandelt wurden. Falls nicht, lässt sich der Code nicht kompilieren.
...
standardgemäß sind alle Elemente in einem Modul *private*. Erst mit dem Hinpufügen des `pub`-Keywords 
...
Abhängikeiten können in der `Cargo.toml`-Datei definiert werden:
...
Die Doku für *crates* und alle Abhängikeiten kann mit dem folgenden Befehl erstellt werden:
...
*Garbage Collector*. Damit ist ein Code gemeint, das zur Laufzeit prüft, ob ein Speicherbereich
...
Eine andere Möglichkeit ist es, die Verantwortung des Speichermanagement komplett an den Entwickler/ 
die Entwicklerin abzugeben. In *C* beispielsweise wird Speicher auf dem Heap mittels `malloc` bzw. `calloc` 
reserviert und muss später mittels des `free` aufrufs wieder explizit freigeben werden. 
...
Freiheiten beim Speichermanagement, ohne auf Laufzeitmechanismen wie einen 
*Garbage Collector* zu setzen.

Bei Rust wird bereits beim Programmüberseten geprüft, ob es zu eventuellen Speicheroblemen kommt. 
Es müssen drei einfache Regeln befolgt werden, damit ein Programm das Prinzip des *Ownerships* nicht verletzt.
...
Beide Variablen würden sonst auf den selben Bereich im Speicher zeigen und falls eine der beiden Variable 
*out-of-scope* geht, müsste der Speicherbereich freigegeben werden.
...
* Viele Probleme werden schon beim Übersetzen erkannt 
